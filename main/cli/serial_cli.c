...
static int cmd_config_show(int argc, char **argv)
{
    ...
    print_config("Device ID",  MIMI_NVS_DEVICE, MIMI_NVS_KEY_DEVICE_ID,  MIMI_SECRET_DEVICE_ID, false);
    print_config("Device Role", MIMI_NVS_DEVICE, MIMI_NVS_KEY_DEVICE_ROLE, MIMI_SECRET_DEVICE_ROLE, false);
    printf("=============================\n");
    return 0;
}
...
static int cmd_config_reset(int argc, char **argv)
{
    const char *namespaces[] = {
        MIMI_NVS_WIFI, MIMI_NVS_TG, MIMI_NVS_LLM, MIMI_NVS_PROXY, MIMI_NVS_SEARCH, MIMI_NVS_DEVICE
    };
    for (int i = 0; i < 6; i++) {
        nvs_handle_t nvs;
...
static int cmd_oled_page(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&oled_page_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, oled_page_args.end, argv[0]);
        return 1;
    }

    const char *arg = oled_page_args.page->sval[0];

    if (strcmp(arg, "auto") == 0) {
        oled_display_set_mode(OLED_MODE_AUTO);
        printf("OLED: auto-rotate mode\n");
        return 0;
    }

    /* Parse as integer page index */
    char *endptr;
    long page = strtol(arg, &endptr, 10);
    if (*endptr != '\0' || page < 0 || page >= OLED_PAGE_COUNT) {
        printf("Error: invalid page '%s'. Use 0-%d or 'auto'.\n",
               arg, OLED_PAGE_COUNT - 1);
        return 1;
    }

    oled_display_set_page((int)page);
    printf("OLED: page %ld — %s (manual mode, 30s timeout)\n",
           page, oled_display_get_page_title());
    return 0;
}

/* --- oled_list command --- */
static int cmd_oled_list(int argc, char **argv)
{
    (void)argc; (void)argv;

    /* Hardcoded page list matching s_pages[] in oled_display.c */
    static const char *titles[OLED_PAGE_COUNT] = {
        "System Status",   "  L1. WiFi Detail",
        "  L2. Memory Detail",
        "AI Agent",        "  L1. Model Info",
        "  L2. Session Info",
        "Network",         "  L1. ESP-NOW Peers",
        "Tasks & Cron",    "  L1. Cron List",
        "  L2. Heartbeat",
        "Messages",
    };

    printf("OLED Menu Pages (%d total):\n", OLED_PAGE_COUNT);
    printf("  auto       — Resume auto-rotate (top-level rotation)\n");
...
static int cmd_set_role(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&set_role_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, set_role_args.end, argv[0]);
        return 1;
    }

    const char *role_str = set_role_args.role->sval[0];
    esp_now_device_role_t role;
    if (strcmp(role_str, "master") == 0) {
        role = ESP_NOW_DEVICE_ROLE_MASTER;
    } else if (strcmp(role_str, "slave") == 0) {
        role = ESP_NOW_DEVICE_ROLE_SLAVE;
    } else {
        printf("Error: role must be 'master' or 'slave'\n");
        return 1;
    }

#if __has_include("espnow/esp_now_device.h")
    esp_err_t err = esp_now_device_set_role(role);
    if (err != ESP_OK) {
        printf("Failed to save role: %s\n", esp_err_to_name(err));
        return 1;
    }
    printf("Device role set to '%s'. Restart to apply.\n", role_str);
#else
...
/* --- set_device_id command --- */
static struct {
    struct arg_str *device_id;
    struct arg_end *end;
} set_device_id_args;

static int cmd_set_device_id(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&set_device_id_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, set_device_id_args.end, argv[0]);
        return 1;
    }

    const char *id = set_device_id_args.device_id->sval[0];
    if (strlen(id) == 0) {
        printf("Error: device ID must not be empty\n");
        return 1;
    }

#if __has_include("espnow/esp_now_device.h")
    esp_err_t err = esp_now_device_set_id(id);
    if (err != ESP_OK) {
        printf("Failed to save device ID: %s\n", esp_err_to_name(err));
        return 1;
    }
    printf("Device ID set to '%s'. Restart to apply.\n", id);
#else
    printf("ESP-NOW is not enabled in this build.\n");
    return 1;
#endif
...
#if __has_include("espnow/esp_now_device.h")
    const char *id = esp_now_device_get_id();
    esp_now_device_role_t role = esp_now_device_get_role();
    const uint8_t *mac = esp_now_device_get_mac();

    printf("Device ID:   %s\n", id[0] ? id : "(not set)");
    printf("Role:        %s\n", esp_now_device_is_master() ? "master" : "slave");
    printf("Role (enum): %d\n", (int)role);
...
esp_err_t serial_cli_init(void)
{
    ...
    /* oled_page */
    oled_page_args.page = arg_str1(NULL, NULL, "<page|auto>", "Page index (0-11) or 'auto'");
    oled_page_args.end  = arg_end(2);
    esp_console_cmd_t oled_page_cmd = {
        .command = "oled_page",
        .help = "Set OLED display page (0-11) or 'auto' for rotating. e.g. oled_page 3",
        .func = &cmd_oled_page,
        .argtable = &oled_page_args,
    };
    esp_console_cmd_register(&oled_page_cmd);

    /* oled_list */
    esp_console_cmd_t oled_list_cmd = {
        .command = "oled_list",
        .help = "List all OLED menu pages",
        .func = &cmd_oled_list,
    };
    esp_console_cmd_register(&oled_list_cmd);

    /* set_role */
    set_role_args.role = arg_str1(NULL, NULL, "<master|slave>", "Device role: master or slave");
    set_role_args.end  = arg_end(1);
    esp_console_cmd_t set_role_cmd = {
        .command = "set_role",
        .help = "Set ESP-NOW device role (master/slave). Takes effect after restart.",
        .func = &cmd_set_role,
        .argtable = &set_role_args,
    };
    esp_console_cmd_register(&set_role_cmd);

    /* set_device_id */
    set_device_id_args.device_id = arg_str1(NULL, NULL, "<id>", "Device identifier (e.g. livingroom)");
    set_device_id_args.end = arg_end(1);
    esp_console_cmd_t set_device_id_cmd = {
        .command = "set_device_id",
        .help = "Set ESP-NOW device ID. Takes effect after restart.",
        .func = &cmd_set_device_id,
        .argtable = &set_device_id_args,
    };
    esp_console_cmd_register(&set_device_id_cmd);

    /* device_info */
    esp_console_cmd_t device_info_cmd = {
        .command = "device_info",
        .help = "Show current device ID, role and MAC address",
