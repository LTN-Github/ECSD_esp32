...
/**
 * 函数名: parse_and_set_time
 * 功能: 解析HTTP日期格式字符串并设置系统时钟，返回格式化的本地时间字符串
 * 参数:
 *      date_str - HTTP日期格式字符串，如"Sat, 01 Feb 2025 10:25:00 GMT"
 *      out - 输出缓冲区，存储格式化的本地时间
 *      out_size - 输出缓冲区大小
 * 返回值: true表示解析和设置成功，false表示失败
 * 作者: Lskipktw
...
/**
 * 函数名: fetch_time_via_proxy
 * 功能: 通过HTTP代理获取时间，向api.telegram.org发送HEAD请求并解析Date头
 * 参数:
 *      out - 输出缓冲区，存储格式化的本地时间
 *      out_size - 输出缓冲区大小
 * 返回值: ESP_OK成功，其他为错误码
...
/* 用于存储Date响应头的上下文结构体 */
typedef struct {
    char date_val[64];  /* 存储Date头的值 */
} time_header_ctx_t;

/**
 * 函数名: time_http_event_handler
 * 功能: HTTP事件处理器，捕获HTTP响应中的Date头信息
 * 参数:
 *      evt - HTTP事件结构体，包含事件类型和用户数据
 * 返回值: ESP_OK始终返回成功
 * 作者: Lskipktw
 * 日期: 2026-04-16  23:36:00
 */
...
/**
 * 函数名: fetch_time_direct
 * 功能: 通过直接HTTPS连接获取时间，向api.telegram.org发送HEAD请求
 * 参数:
 *      out - 输出缓冲区，存储格式化的本地时间
 *      out_size - 输出缓冲区大小
 * 返回值: ESP_OK成功，其他为错误码
 ...
 * 功能: 执行获取当前时间操作，自动选择代理或直接连接方式获取时间
 * 参数:
